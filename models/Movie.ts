...
export class Movie {
  ...
  borshInstructionLayout = borsh.struct([
    borsh.u8("variant"),
    borsh.str("title"),
    borsh.u8("rating"),
  ...
  // Construct the bytes that we will send to the Solana smart contract
  serialize(): Buffer {
    const buffer = Buffer.alloc(1000);
    this.borshInstructionLayout.encode({ ...this, variant: 0 }, buffer);
    return buffer.slice(0, this.borshInstructionLayout.getSpan(buffer));
  }
}
